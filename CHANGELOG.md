...
- Renamed "Presence trigger" switch to "Device presence trigger". Please check your setting for this switch.
- Renamed "End on condition" switch to "Exit condition trigger". Please check your setting for this switch.
...
### Fix
- Fixed issue with invalid power factor validation.
...
- Global defaults "Delta allocated power" now shows power available to SC for allocation.
...
### Improvement
- Enabled polling by default for Tesla Fleet, Tessie and Teslemetry.
...
- Added [Pause on start](https://github.com/flashg1/SolarCharger/wiki/User-guide#pause-on-start) switch to monitor available solar for device that uses a lot of energy when switched on.
